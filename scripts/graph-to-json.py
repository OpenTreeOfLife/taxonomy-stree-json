...
print "Loading taxonomy into graph..."
g = tg.load_taxonomy_graph('ncbi/ncbi.xml.gz')
print "Taxonomy Graph Complete."




datafile = open('treecache.txt', 'r') #read in the treecache file
data = []
errors = []
blacklist = []
...
tree_blacklist = open('tree_blacklist.txt', 'r') #read in the tree blacklist file
    ...
    if active_tree[0] in blacklist: ## if a tree is in the blacklist, ignore it.
        ...
        if leafcount <= 5000: #check to prune trees that have more than 5000 leaves. They will not display correctly in graph form.
            ...
            try: #used to catch all errors from incorrectly formatted trees (ie: ? characters, and other issues)
                ...
                date = str(time.strftime("%x")) # grab the system date for the filename and convert it to a string
                date = date.replace("/", "_") # parse it to be more file_name friendly
                treeid = str(stree) # convert stree int into a string
                filename = "treegraphjson/ncbi/" + date + "_tree_" + treeid + ".JSON" # build the full file_name for writing
                f = open(filename, 'w')
                f.write(result)
                f.close
                print "Done."
